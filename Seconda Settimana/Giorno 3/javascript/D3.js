...
for(let j = 0;j<starWarsCharacters.length;j++){
  if (starWarsCharacters[j].gender === "female"){
    femaleCharacters.push(starWarsCharacters[j])
  }
}
...
for(let k = 0; k < starWarsCharacters.length;k++){
  let character = starWarsCharacters[k]  // è sempre bene creare una variabile o const per non lavorare direttamente sull'array
